<form id="NewMemberForm" action="{{ isset($user) ? route('hof.add.member', ['user' => $user->crypt_id]) : route('hof.store') }}" method="POST" novalidate enctype="multipart/form-data" class="member_form">
	<div class="row">
		
		<div class="col-lg-12 master-board">
			<div class="card">
				<div class="card-header align-items-center d-flex">
					<h4 class="card-title mb-0 flex-grow-1">Member Personal Details</h4>
				</div>
				<div class="card-body">
				 
					@csrf
					<input type="hidden" name="tab" value="new-member-form-tab"/>
					<input type="hidden" name="hof" value="{{$user->crypt_id}}"/>
						...
						<div class="col-lg-3">
							<div class="mb-5">
								...
								<input class="form-control @error('first_name') is-invalid @enderror" id="first_name" name="first_name" type="text"
									placeholder="Surname" value="{{ old('first_name') ?? 'Patel' }}">
								@error('first_name')
								<label class="error m-t-1 text-danger">{{ $message }}</label>
								@enderror
							</div>
					...
					<div class="row">
						<div class="col-lg-3">
							<div class="mb-5">
								...
								@php
								$selectedRelation = old('relation');
								@endphp
								<select class="form-select mb-5 " name="relation">
									@foreach ($relations as $relation)
									@if($relation->id != \App\Models\HofRelation::RELATION_SELF)
									<option value="{{$relation->id}}" {{($selectedPrefix==$relation->id) ? "selected='selected'" : "" }}>{{$relation->title}}</option>
									@endif
									@endforeach
								</select>
								@error('relation')
						...
						<div class="col-lg-5">
							<div class="mb-5">
								...
								<input class="form-control @error('email') is-invalid @enderror" id="email" name="email" type="email"
									placeholder="fullname@mail.com" value="{{ old('email') }}">
								@error('email')
								<label class="error m-t-1 text-danger">{{ $message }}</label>
								@enderror
							</div>
							...
							<div class="mb-5">
								...
								<div class="input-group" data-input-flag id="phone_code" data-url="{{asset('')}}">
									...
									<select name="country_code" style="max-width: 70px;" class="form-control">
										...
										@php 
											$selectedPhoneCode = old('country_code') === $phone_code ? 'selected' : ($phone_code
											== '+91' ? 'selected' : '');
										@endphp
											<option {{$selectedPhoneCode}} value="{{$phone_code}}">{{$phone_code}}</option>
										@endforeach
									</select>
									<input type="text" class="form-control rounded-end flag-input" value="{{ old('phone') }}" name="phone" placeholder="Enter number" id="phone"
						...
						<div class="col-lg-4">
							<div class="mb-5">
								...
								<input type="text" class="form-control flatpickr-input active" data-provider="flatpickr" data-date-format="d M, Y"
									data-mindate="25 12, 2021" value="{{ old('dob') }}" name="dob" readonly="readonly">
							...
							<div class="mb-5">
								...
								<input class="form-control @error('weight') is-invalid @enderror" id="weight"
									name="weight" type="text" placeholder="55"
									  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" maxlength="3" value="{{ old('weight') }}">
						...
						<div class="col-lg-4">
							<div class="mb-5">
								<label for="height">Select Height<span class="text-danger">*</span></label>
								 @php
								$selectedHeight = old('height') ?? "5'6\"";
								@endphp
								<select class="form-select mb-5 js-example-basic-single" name="height">
									@foreach ($human_heights as $human_height)
									<option value="{{$human_height}}" {{($selectedHeight==$human_height) ? "selected='selected'" : "" }}>{{$human_height}}
									</option>
									@endforeach
								</select>
								@error('height')
					...
					<div class="row">
						<div class="col-lg-4">
							<div class="row mb-2">
								...
								@php
								$selectedGender = old('gender') == $key ? 'checked' : ($key
								== 1 ? 'checked' : '');
								@endphp
									<div class="col-lg-4">
										<input class="form-check-input" value="{{$key}}" type="radio" {{$selectedGender}} name="gender" id="{{$gender}}-{{$key}}-new-member">
										<label class="form-check-label" for="{{$gender}}-{{$key}}-new-member">
											 {{$gender}}
										</label>
									</div>
								@endforeach
							...
							<div class="row mb-2">
								<div class="mt-5">
									...
									@php
									$selectedBloodGroup = old('blood_group');
									@endphp
						...
						<div class="col-lg-4">
							<div class="row mb-2">
								<div class="mb-1">
									...
									@php
									$selectedMaritalStatus = old('marital_status');
									@endphp
							...
							<div class="row mb-2">
								<div class="mt-4">
									...
									@php
									$selectedMemberStatus = old('status');
									@endphp
			...
			<div class="card">
				<div class="card-header align-items-center d-flex">
					<h4 class="card-title mb-0 flex-grow-1">Member Education and Occupation Details</h4>
				</div>
				<div class="card-body">
					<div class="row">
						<div class="col-lg-6">
							<div class="mb-5">
								...
								@php
								$selectedOccupation = old('occupation_id');
								@endphp
						...
						<div class="col-lg-6">
							<div class="mb-5">
								<label for="occupation_detail">Occupation Detail <span class="text-danger">*</span></label>
						...
						<div class="col-lg-6">
							<div class="mb-5">
								...
								@php
								$selectedEducation = old('native_village_id');
								@endphp
						...
						<div class="col-lg-6">
							<div class="mb-5">
								<label for="education_detail">Member Education Detail <span class="text-danger">*</span></label>
			...
			<div class="card" id="new-member-address">
				<div class="card-header align-items-center d-flex">
					<h4 class="card-title mb-0 flex-grow-1">Member Address Details</h4>
					<a href="javascript:void(0)" data-section="new-member-address" class="btn btn-info address-clone">Same As HOF</a>
				</div>
				<div class="card-body">
				<div class="row">
					<div class="col-lg-3">
						<div class="mb-5">
							...
							@php
							$selectedNative =  old('native_village_id');
							@endphp
				...
				<div class="row">
					<div class="col-lg-4">
						<div class="mb-5">
							...
							@endphp
							<select class="form-select mb-5 js-example-basic-single country_id" name="country_id" data-url="{{route('location.countries')}}" data-selected="{{$selectedCountry}}" data-id="new-member" id="country_id_new-member">
					...
					<div class="col-lg-4">
						<div class="mb-5">
							...
							@endphp
							<select class="form-select mb-5 js-example-basic-single state_id" name="state_id" data-url="{{route('location.states')}}" data-selected="{{$selectedState}}" data-id="new-member" id="state_id_new-member">
					...
					<div class="col-lg-4">
						<div class="mb-5">
							...
							@php
							$selectedState =  old('city_id');
							@endphp
							<select class="form-select mb-5 js-example-basic-single city_id" name="city_id" data-url="{{route('location.cities')}}" data-selected="{{$selectedState}}" data-id="new-member" id="city_id_new-member">
							</select>
							@error('city_id')
							<label class="error m-t-1 text-danger">{{ $message }}</label>
							@enderror
